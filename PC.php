...
<div id="backIndex">
	...
	<div class="" style="text-align: center; top: 120px; color: #fff; width: 100%; position: fixed;"  data-anijs="if: load, on: window, do: fadeInUp animated">
		<img src="https://bj.bcebos.com/v1/leafer/Starry/img/lang-zh/best-couple-title.png" style="width: 420px;" />	
	</div>
	
	<div class="deepIndex" style="top: 50%; transform: translateY(-60%) translateX(-50%); left: 50%; ">

		<span style = "color: #ffffff; font-size: 350%" id="CoupleName"> 正在探寻中 </span>
		
	</div>
	
	<div class="deepIndex" style="top: 70%; transform: translateY(-60%) translateX(-50%); left: 50%; ">

		<span style = "color: #ffffff; font-size: 350%" id="CoupleVote">  </span>
	...
	<div class="" style="text-align: left; top: 150px; color: #fff; width: 100%; position: fixed;"  data-anijs="if: load, on: window, do: fadeInUp animated">
		<img src="https://bj.bcebos.com/v1/leafer/Starry/img/lang-zh/prom-king-title.png" style="width: 420px;" />	
	</div>
	

	<div class="deepIndex" style="top: 50%; transform: translateY(-60%) translateX(-50%); left: 15%; ">

		<span style = "color: #ffffff; font-size: 320%" id="KingName"> 正在探寻中 </span>
		
	</div>
	
	<div class="deepIndex" style="top: 65%; transform: translateY(-60%) translateX(-50%); left: 15%; ">

		<span style = "color: #ffffff; font-size: 320%" id="KingVote"> 0 </span>
		
	</div>
	
	<!--right-->	
	
	<div class="" style="text-align: right; top: 150px; color: #fff; width: 100%; position: fixed;"  data-anijs="if: load, on: window, do: fadeInUp animated">
		<img src="https://bj.bcebos.com/v1/leafer/Starry/img/lang-zh/prom-queen-title.png" style="width: 420px;" />	
	</div>
	

	<div class="deepIndex" style="top: 50%; transform: translateY(-60%) translateX(-50%); left: 85%; ">

		<span style = "color: #ffffff; font-size: 320%" id="QueenName"> 正在探寻中 </span>
		
	</div>
	
	<div class="deepIndex" style="top: 65%; transform: translateY(-60%) translateX(-50%); left: 85%; ">

		<span style = "color: #ffffff; font-size: 320%" id="QueenVote"> 0 </span>
...
<script>
		...
		var King_current = <?php echo $KingVote?>;
		var KingCounter = new CountUp('KingVote', King_current * 0.996, King_current, 0, 50);
		KingCounter.start();
		
		var Queen_current = <?php echo $QueenVote?>;
		var QueenCounter = new CountUp('QueenVote', Queen_current * 0.996, Queen_current, 0, 50);
		QueenCounter.start();
		
		var Couple_current = <?php echo $CoupleVote?>;
		var CoupleCounter = new CountUp('CoupleVote', Couple_current * 0.996, Couple_current, 0, 50);
		CoupleCounter.start();
		
		setInterval(function(){
			$.ajax({url:"getBest.php",
				data: {},
				dataType: 'JSON',
				timeout: 3000,
				type: 'POST',
				success: function(res){
					...
					KingCounter.update(res.KingVote);
					QueenCounter.update(res.QueenVote);
					CoupleCounter.update(res.CoupleVote);
					$('#KingName').html(res.KingName);
					$('#QueenName').html(res.QueenName);
					$('#CoupleName').html(res.CoupleName);
...
<div class="coverIndex" style="" id="login">
	...
	<div id="loginPanel" style="padding: 0px 40px;">
		...
		<input id="t-code" data-lang="login-password-input" class="layui-input" placeholder="教师授权码" type="password" style="text-align: center;"></input>
		
		<div style="text-align: center; padding-top: 15px;">
			<a id="loginStud" class="layui-btn layui-btn-lg"><i class="fa fa-lock"></i>&nbsp;&nbsp;<span data-lang="login-button">访问授权</span></a>
		</div>
...
<div id="table-frame" style="position: fixed; top: 50%; left: 50%; transform: translateX(-50%) translateY(-50%); width: 60%; overflow-y: hidden;">
	<div class="items-static" id="table"></div>
</div>
		...
		success: function(res){
			if(res.code!=0){
				layui.use('layer', function(){
					...
					layer.msg($.i18n.prop('login-fail'));
				});
				loginProcessSwitch('Panel');
			}else{
				//登陆成功
				
				$('#table-frame').fadeOut(250);
				...
				$.ajax({url:"teacher.php",
					...
					success: function(res){
						...
						bgSwitch('2');
						$('#backIndex').fadeOut(250);
						$('#table-frame').fadeIn(250);
					},
					error: function (xhr,status,error){
						$('#loginStud').click();
						layui.use('layer', function(){
