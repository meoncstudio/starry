<?php
	...
	if(!empty($func)){
		...
		if($func == 'login'){
			...
			$temp = explode(' ', $un);
			$conselorFirstName = $temp[0];
			$conselorLastName = $temp[1];
			...
			<div class="items-title">
				<span data-lang="account">账户</span> <b><?php echo  $conselorFirstName . ' ' . $conselorLastName ?></b> <span data-lang="student-list">学生名单</span>: 
				
				<a id="refreshTable" class="layui-btn layui-btn-sm" style="float: right;" data-lang="refresh">刷新</span></a>
			</div>
			...
			<table lay-filter="table">
			  <thead>
				<tr>
				  <th lay-data="{field:'no', width: 50, sort: true}">No.</th>
				  <th lay-data="{field:'username', width: 100, sort: true}"><span data-lang="login-username-input">用户名</span></th>
				  <th lay-data="{field:'name', width: 200, sort: true}"><span data-lang="name">姓名</span></th>
				  <th lay-data="{field:'type', width: 150, sort: true}"><span data-lang="ticket-type-short">票型</span></th>
				  <th lay-data="{field:'status', width: 120, sort: true}"><span data-lang="status">状态</span></th>
				  <th lay-data="{field:'active', width: 180, sort: true}"><span data-lang="purchase-time">购买时间</span></th>
				</tr> 
			  </thead>
			  <tbody>
			...
			if($conselorFirstName == 'Business' && $conselorLastName == 'Department'){
				
				$ret2 = SQL("SELECT `username`, `type`, `status`, `purchaseTime` FROM `graduation_prom_2018_tickets` WHERE `type` = 'ACADEMY' ");
				$studs = array();
				$tt = -1;
				
				for($t = 0; $t < count($ret2); $t ++){
					
					$username = $ret2[$t]['username'];
					$ret = SQL("SELECT `aid`, `username`, `firstName`, `middleName`, `lastName` FROM `graduation_prom_2018_account` WHERE `username` = '$username'   ");
					
					if(!empty($ret)){
						
						$tt ++;
						$studs[$tt] = array_merge($ret2[$t], $ret[0]);
				...
				$ret = SQL("SELECT `aid`, `username`, `firstName`, `middleName`, `lastName` FROM `graduation_prom_2018_account` WHERE `conselorFirstName` = '$conselorFirstName' AND `conselorLastName` = '$conselorLastName'   ");
				...
				for($t = 0; $t < count($ret); $t ++){
					...
					$ret2 = SQL("SELECT `type`, `status`, `purchaseTime` FROM `graduation_prom_2018_tickets` WHERE `username` = '$username' ");
					
					if(!empty($ret2)){
						
						$tt ++;
						$studs[$tt] = array_merge($ret[$t], $ret2[0]);
			...
			for($t = 0; $t < count($studs); $t ++){
				
				
					?>
						<tr>
							<td><?php echo $studs[$t]['aid'] ?></td>
						  <td><?php echo $studs[$t]['username'] ?></td>
						  <td><?php echo $studs[$t]['lastName'] . ', ' . $studs[$t]['firstName'] . ' ' . $studs[$t]['middleName'] ?></td>
						  <td><span data-lang="<?php echo strtolower($studs[$t]['type']) ?>-ticket"></span></td>
						  <td><span data-lang="status-<?php echo strtolower($studs[$t]['status']) ?>"></span></td>
						  <td><?php echo $studs[$t]['purchaseTime'] ?></td>
						</tr>
			...
			星空夏 Starry | Copyright &copy; Cloudy Young 2018
